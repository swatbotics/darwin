...
function votingResponse()
{
    if (ajaxReady()) {
        if (http.responseText.indexOf('invalid') == -1) {
            ...
            var yes = xmlValue(xmlDocument, 'yes');
            var no = xmlValue(xmlDocument, 'no');
...
function voting(id, yes_no)
{
    var data = new HTTPParam().create('ajax_voting_' + yes_no, id);
    ajaxSend(data, votingResponse);
}
